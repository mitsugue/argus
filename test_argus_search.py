...
MASTER = [
    {"code4": "1346", "ja": "MAXIS 日経225 ETF", "en": "MAXIS NIKKEI 225 ETF", "mkt": "ETF"},
...
def test_jp_name_query_surfaces_matching_stocks_not_etf(monkeypatch):
    monkeypatch.setattr(scanner, "_jq_master", lambda: MASTER)
    res, _ = scanner._search_jp("三菱")
    syms = [r["symbol"] for r in res]
    # both 三菱 names present; the unrelated 三井 must not appear
