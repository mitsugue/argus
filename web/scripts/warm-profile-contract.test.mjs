...
assert.match(seedAction, /ARGUS_REQUIRE_LIVE_CANDIDATE/);
assert.match(deploy, /candidate-identity:/);
assert.match(deploy, /needs: \[scope, candidate-identity, business-snapshot-trigger, acceptance-runtime-admission\]/);
assert.match(deploy, /enforce-public-candidate-identity: 'true'/);
assert.match(acceptance, /name: zero-install-runtime-proof-1/);
assert.match(acceptance, /name: zero-install-runtime-proof-2/);
assert.match(acceptance, /full-release-simulation\.mjs --run 1/);
assert.match(acceptance, /full-release-simulation\.mjs --run 2/);
assert.doesNotMatch(acceptance, /warm-profile-handoff:/);
