...
assert.match(deploy, /ARGUS_EXPECTED_SHA: \$\{\{ github\.sha \}\}/);
assert.match(deploy, /warm-profile-contract\.mjs sanitize-validate/);
assert.match(deploy, /warm-profile-contract\.mjs validate/);
...
assert.match(acceptance, /ARGUS_ACCEPTANCE_MODE: profile/);
assert.match(acceptance, /actions\/download-artifact@v5/);
