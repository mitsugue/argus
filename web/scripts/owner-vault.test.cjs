...
(async()=>{const data=Object.fromEntries([...values].map(([k,v])=>[k,k==='argus.locale.v1'?v:JSON.parse(v)]));const receipt={snapshotId:'a'.repeat(64),contractVersion:2,dataHash:await api.digest(JSON.stringify(data)),savedAt:Date.now()/1000,exportedAt:new Date().toISOString()};
