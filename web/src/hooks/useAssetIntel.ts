...
export function useAssetIntel(opts: {
  ...
  const newsIntel = useNewsIntelligence().view;   // v13.5.35 news signal
  ...
  const canonicalDecision = useMemo(() => {
    ...
    // v13.5.35 (review item C): uncapped imminent feed + impact tiering.
    // critical/high → WAIT_REQUIRED; medium/low → BLOCK_BUY only.
    const eventGate = imminentEventGate(impEvents);
    // v13.5.35 NEWS/EVENT SIGNAL → execution constraint, PER SUBJECT
    ...
    for (const asset of decisionSubjects) {
      ...
      // v13.5.35 (external review item 7): a calendar FEED outage is an
      ...
      // v13.5.35 (review item F): the old composite lever data-gated EVERY
