...
export async function vaultRequest(token:string,vaultId:string,operation:string,fields:Record<string,unknown>={}){
  const base=(import.meta.env.VITE_ARGUS_BACKEND_URL as string|undefined)?.replace(/\/$/,'');
  if(!base)throw new Error('保存先に接続できません。');
  const abort=new AbortController();const timer=setTimeout(()=>abort.abort(),190000);
  try{
...
export async function saveOwnerVault(token:string,pass:string,onProgress:(message:string)=>void,payload=buildBackupPayload()):Promise<VaultSnapshot>{
  ...
  const pending=await pendingVault(vaultId);let blob:string;
  ...
  else{blob=await encryptBackup(pass,payload);await pendingVault(vaultId,{vaultId,dataHash,blob});}
  ...
  const begin=await vaultRequest(token,vaultId,'begin',{snapshotId,bytes});
  ...
  if(begin.state!=='VERIFIED'){
    ...
    for(let offset=0;offset<count;offset+=4){await Promise.all(Array.from({length:Math.min(4,count-offset)},async(_,i)=>{
      const part=offset+i;if(!completed.has(part))await vaultRequest(token,vaultId,'chunk',{snapshotId,part,data:blob.slice(part*16000,(part+1)*16000)});
    }));onProgress(`暗号化データを転送中 ${Math.min(count,offset+4)}/${count}`);}
    let state=await vaultRequest(token,vaultId,'commit',{snapshotId});const deadline=Date.now()+210000;
    while(state.state!=='VERIFIED'){
      if(state.state==='FAILED'||state.state==='INTERRUPTED'||Date.now()>deadline)throw new Error('遠隔保存を確認できていません。現在の端末データはそのままです。');
      onProgress('非公開の保存先へ記録し、内容を読み戻しています。');
      await new Promise(resolve=>setTimeout(resolve,2000));state=await vaultRequest(token,vaultId,'status',{snapshotId});
    }
  }
  onProgress('保存点を読み戻し、この端末で復号して照合しています。');
  const restored=await readOwnerVault(token,pass,snapshotId);
  ...
  try{localStorage.setItem(OWNER_VAULT_RECEIPT,JSON.stringify({...receipt,contractVersion:BACKUP_CONTRACT_VERSION,dataHash:await digest(JSON.stringify(payload.data))}));}catch{/* Remote proof still exists. */}
  await pendingVault(vaultId,null);
  window.dispatchEvent(new CustomEvent('argus:vault-saved'));
  return receipt;
}
export async function readOwnerVault(token:string,pass:string,snapshotId:string):Promise<BackupFile>{
  if(!/^[a-f0-9]{64}$/.test(snapshotId))throw new Error('保存点の識別情報が不正です。');
  const value=await vaultRequest(token,await vaultIdFrom(pass),'read',{snapshotId});
