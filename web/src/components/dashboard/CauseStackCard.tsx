...
export const CauseStackCard: React.FC<{ symbol: string; market?: string }> = ({ symbol, market = 'JP' }) => {
  ...
  async function research() {
    // v11.3.3: cached-only — the public GET no longer fires a live AI search.
    // Explanations are generated by the admin cron for the biggest unresolved movers.
    ...
    <section className="csc-card">
      ...
      <div className="csc-why">
        <button className="csc-why-btn" disabled={why.loading} onClick={research}>
          {why.loading ? '🔎 取得中…' : '🔎 AI解説を表示(定期生成・キャッシュ)'}
        </button>
        {why.text && <p className="csc-why-text">{why.text}</p>}
        {why.text && <p className="csc-why-note">AI解説は管理側の定期生成のみ。要確認・投資助言ではありません。</p>}
      </div>
