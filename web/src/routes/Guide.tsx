...
const CAPABILITIES: { area: string; descJa: string }[] = [
  ...
  { area: 'ダウンサイド対応 (急落の理由と対応)',
    descJa: '保有/監視銘柄が急落(-3%/-5%等)、または日本市場の地合いが悪化したとき、ARGUSは「急落しています」で終わらせず、原因を9分類で確率推定(市場全体/セクター/テーマ利確/個別悪材料/大口売り/踏み上げ一巡/急騰後利確/テクニカル崩れ/原因未確認)し、保有向けにアクションを上書き提示する。原因未確認の急落は安全とみなさず警戒を上げ、大きな不明下落は通常のHOLDにしない(REVIEW REQUIRED/買い増し禁止/縮小・撤退の検討など)。保有銘柄は一段厳しめ。地合いはグローバルがRISK_ONでも日本のザラ場が弱ければ別オーバーレイ(JP intraday CAUTION)で表示。通知も銘柄/下落率/原因/上書き/次の確認条件入り。日本株はTDnet適時開示(下方修正/減配/増資等)を自動取得して原因に反映(v10.101+)。自動売買は一切しない(決定支援のみ)。' },
