...
const CAPABILITIES: { area: string; descJa: string }[] = [
  ...
  { area: 'バックアップ',
    descJa: 'パスフレーズを1度設定すれば、毎日自動で暗号化バックアップがクラウド(GitHub)に保存される(端末上で暗号化・暗号文しか外に出ない・直近8世代を自動保持)。新端末はパスフレーズだけで復元。週1回のローカルファイル保存と手動エクスポート/復元も併用可。' },
  { area: '判断ログ (記憶)',
    descJa: '毎日の判断を端末内に記録し「昨日からの変化」と直近7日の判断を表示(この端末のみ)。' },
  { area: '予測台帳と自己採点',
    descJa: '毎営業日16:05に「個別銘柄+9資産クラス(SPY/QQQ/GLD/TLT/REIT等)+BTC/ETH+姿勢の判断そのもの」を記録し、翌日に実値と照合して自動採点。個別株の暗記ではなく「局面×資産クラス」の校正を蓄積する設計(姿勢はSPYの実際の動きで採点)。成績はTodayに表示。' },
  { area: 'AI予想',
...
const RECENT_UPDATES: [string, string][] = [
  ...
  ['v10.4.0', 'Action Alertsページの実データ化 — 金/債券/REIT/仮想通貨/USDJPY/現金/日米株の8クラスをライブ判断(最後の主要mockが解消)。Index Fundsも実際の保有投信+姿勢連動に'],
  ['v10.3.4', 'クラウド自動バックアップ — パスフレーズ設定だけで毎日自動・端末上で暗号化・新端末はパスフレーズのみで復元。古い世代は自動削除'],
