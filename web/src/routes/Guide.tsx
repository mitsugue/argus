...
const RECENT_UPDATES: [string, string][] = [
  ['v10.205.0', '日本株の自己採点が「採点待ち」で止まる根本原因を修正 — 深掘り検証で判明: 日次ランは16:05 JST(東証引け後)に走るが、その時刻の日本株終値は「delayed」扱いで、価格取得が「liveのみ」だったため日本株が毎回除外され、記録も採点もされていなかった(=永遠に採点待ち。US/暗号は元々クロック未実装で保留)。引け後の確定終値は日次台帳の正しいアンカーなので、mock以外の実価格(live/delayed/確定終値)を受け入れるよう修正。今後の日次ランから日本株が記録・採点され、対象営業日の到来分から成績が入り始めます。'],
