...
const PAGE_GUIDE: { page: string; descJa: string }[] = [
  ...
  { page: '① Today(今日の判断)',
    descJa: 'まずここを開く。市場ランプで今どの市場が開いているか→総合判断(HOLD等/リスク/理由/触る・避ける・次の条件)で今日の構え→イベント(市場セッション対応・暗号は24/7)でS高/急変の有無(タップで「何が起きた/原因/シナリオ/罠」)→判断ログで自己採点の成績。急落時は「Downside Watch」が原因推定と保有向けの対応(通常HOLDにしない)を表示。「BIG MONEY / FLOW」は本日の大きな動きが誰の売買か(大口買い集め/買い戻し/個人追随などの可能性)を証拠ベースで推定(v11.7・売買指示なし)。「PORTFOLIO EXPOSURE」は保有の集中・テーマ偏り・買い増し余地を点検(v11.8・端末内計算)。10秒で全体把握。' },
  { page: '② Watchlist(個別銘柄)',
