...
const RECENT_UPDATES: [string, string][] = [
  ['v10.18.0', '⚡診断に信用残(信用買い残・売り残)を統合 — 「大口の動きは買い戻しか新規買いか」の問いに接近。信用倍率(売り長=踏み上げ余地/買い長=戻り売り圧力)と前週比の増減を判定。J-Quantsのプランで取れない場合は正直に「未提供」と表示し誤データは出さない'],
  ['v10.17.0', '⚡診断にMACD・ボリンジャーバンド・ゴールデン/デッドクロスを統合(RSIは実装済み) — 古典テクニカルが揃い、根拠は全件表示のまま。AI Reviewの銘柄に日本語社名を表示'],
  ['v10.16.1', 'ナビを判断の流れ順に再設計 — 1.Today(全体把握) → 2.Watchlist(個別に入る・⚡診断) → 3以降は補助情報(Action Alerts/Regime/Event Radar/司令室)。スクロールのページ送りも同じ順序に'],
  ...
  ['v10.15.0', '⚡エントリー診断(entry-scout) — 銘柄の戦略カードから1タップで「入りの瞬間診断」: 60日履歴のトレンド/過熱度+大口フロー+イベント接近+地合いを束ねて理由つきで即答。日本株のみ対応'],
  ['v10.14.0', 'ニュース見出しの日本語化+Event Radarの配置改善 — Market Newsの見出しを自動翻訳(原文はホバーで表示)、地政学キーワード(イラン/台湾/攻撃等)も⚡判定に追加。売買判断の主材料であるEvent RadarをTodayの上部(判断ログ直下)へ移動'],
