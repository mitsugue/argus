...
const PAGE_GUIDE: { page: string; descJa: string }[] = [
  ...
  { page: '① Today(今日の判断)',
    descJa: 'まずここを開く。市場ランプで今どの市場が開いているか→総合判断(HOLD等/リスク/理由/触る・避ける・次の条件)で今日の構え→イベント(市場セッション対応・暗号は24/7)でS高/急変の有無(タップで「何が起きた/原因/シナリオ/罠」)→判断ログで自己採点の成績。急落時は「Downside Watch」が原因推定と保有向けの対応(通常HOLDにしない)を表示。「BIG MONEY / FLOW」は本日の大きな動きが誰の売買か(大口買い集め/買い戻し/個人追随などの可能性)を証拠ベースで推定(v11.7・売買指示なし)。「PORTFOLIO EXPOSURE」は保有の集中・テーマ偏り・買い増し余地を点検(v11.8・端末内計算)。「SUPPLY / DEMAND」は日本株の需給をランクS〜Eで表示(v11.10・生数値は折りたたみ)。最上部の「SESSION BRIEF」は今日の作戦(攻める/待つ/守る/監視/記録の日・やらないこと・次の確認 v11.13)。上部の「ACTION PRIORITY」は全レイヤー統合の「今日これを見る」リスト(v11.12・P0=保有×複合悪材料のみ)。10秒で全体把握。' },
  { page: '② Watchlist(個別銘柄)',
  ...
  { page: '④ Core Portfolio(資産配分)',
    descJa: '①あなたの実配分(円換算・含み損益) ②8資産クラスのライブ判断(クラス判断) ③姿勢連動の積立方針 ④投信の基準価額(NAV・前日比)を日次フォロー(eMAXIS Slim等。投信総合ライブラリーから取得)。「比率をどう動かすか」の意思決定はこのページで完結。v11.8で「EXPOSURE DASHBOARD」(テーマ別配分・通貨偏り・銘柄集中・リスクフラグ)を追加(端末内計算・売買指示なし)。v11.9で「PORTFOLIO SYNC & BACKUP」(バックアップJSON書き出し/読み込み・日次スナップショット・保存モード表示)を追加。v11.11で「DECISION QUALITY」(過去判断の答え合わせ・行動注釈・端末内のみ)を追加。' },
  { page: '⑤ Guide(このページ)',
...
const CAPABILITIES: { area: string; descJa: string }[] = [
  ...
  { area: 'AI(画面判断の主役・v10.160+)',
    descJa: '判断の主役はAI。GPT-5.5が主判定、Geminiが検算(第二意見)。★銘柄カードのHOLD/EXIT/買い増し等は、AI判定が最新の時はAIの裁定結果を表示し、カードに「AI」バッジが付く。AIが古い/未実行/予算停止の時だけルールエンジンが「ルール暫定」(=画面が空にならない安全網)として出る。AIは判断時に必ず①自己採点(過去成績=教科書)②C.A.O.S.(機関見解)③チャート(RSI/トレンド)④日証金/信用残 を参照する。チェッカーはコスト最適に使い分け: 市場時間中15分毎+場外=Flash、日次16:05の採点される本判定=Pro。結果は毎回 /tmp と台帳に永続化し、サーバー再起動後も直近のAI見解を自動復元(v10.163+)。コストはハード上限(日次$5/月次$80・既定)で制御し上限で新規実行を拒否。GPT-5.5 Pro Handoff(手動コピペ・無料の深掘り相談)も別途あり。下のAPI statusに現在の状態が常に出る。' },
