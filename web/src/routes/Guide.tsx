...
const CAPABILITIES: { area: string; descJa: string }[] = [
  ...
  { area: 'Watchlist',
    descJa: '銘柄を検索して追加(日本株/米国株/投信/暗号資産)・ドラッグ並べ替え。追加した銘柄には自動でライブ価格+ルールベースの行動ラベル+戦略カード(理由/次の条件/シナリオ確率)が付く。日本株はカード内の「⚡エントリー診断」で入りの瞬間判断(トレンド/過熱/大口フロー/イベント)を即取得(v10.15+)。' },
...
const RECENT_UPDATES: [string, string][] = [
  ['v10.26.0', '市場時間の認識(価格の正直化) — 東証の取引時間外(土日・引け後・昼休み)はブリッジが送る価格を「live」ではなく「delayed(前日/引け終値)」と表示。休場中に「最新」と偽る問題を解消。※リアルタイム性はmoomooのデータ契約に依存(遅延配信の場合は別途確認が必要)'],
  ['v10.25.0', 'Gemini OSINT連携(手動ブリッジ) — 消費者版Gemini(Deep Research)はOSINTが強いがAPI非対応のため、銘柄カードの「🔮 Gemini OSINT」ボタンがその銘柄の調査プロンプト(最新ニュース/機関の動き/材料の強弱+確率配分の依頼)をコピー。Geminiアプリに貼って深掘り→「📝リサーチメモ」に結論を貼り戻して保存(端末内・同期)。GPT/Gemini両方の頭脳をコスト0でARGUSに'],
  ['v10.24.0', '校正ループ完成(Phase 3) — ⚡診断のスコアとフロー分類を毎日台帳に記録し、数日後の実値で自動採点。「scoreが高い局面は過去何%上がったか」「買い戻し主導と推定した時に失速したか」が蓄積され、診断に『この水準の実績: 過去N件中X%上昇』として表示(20件貯まるまで参考)。推定が実績に裏打ちされた確率へ'],
  ['v10.23.0', 'チャートの窓(ギャップ)検知+マイトレード記録 — ⚡診断が直近の窓(上放れ/下放れ・埋め/未埋め)を認識し下値支持/上値抵抗として織り込み。Watchlistに「My Trades」: 自分の売買と根拠を記録→ライブ損益と「あなたの判断 vs 結果」を蓄積(端末内・同期対象)。ダブルボトム等の形状認識は引き続き未対応(正直表示)'],
