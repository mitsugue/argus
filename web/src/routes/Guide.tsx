...
const PAGE_GUIDE: { page: string; descJa: string }[] = [
  ...
  { page: '④ Core Portfolio(資産配分)',
    descJa: '①あなたの実配分(円換算・含み損益) ②8資産クラスのライブ判断(クラス判断) ③姿勢連動の積立方針 ④投信の基準価額(NAV・前日比)を日次フォロー(eMAXIS Slim等。投信総合ライブラリーから取得)。「比率をどう動かすか」の意思決定はこのページで完結。v11.8で「EXPOSURE DASHBOARD」(テーマ別配分・通貨偏り・銘柄集中・リスクフラグ)を追加(端末内計算・売買指示なし)。v11.9で「PORTFOLIO SYNC & BACKUP」(バックアップJSON書き出し/読み込み・日次スナップショット・保存モード表示)を追加。' },
  { page: '⑤ Guide(このページ)',
