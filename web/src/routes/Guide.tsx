...
const CAPABILITIES: { area: string; descJa: string }[] = [
  ...
  { area: 'FIRE Core / 投資信託トラッカー (v11.19.1)',
    descJa: 'オーナー方針:「投資信託の合計額をFIRE用の本丸資産として扱います。個別株の利益は、将来的にこのFIRE Coreへ移す候補として見ます。」— ARGUSは全資産を同列に扱わず、投信をFIREの土台として明示追跡します。★Core Portfolioの「FIRE CORE / MUTUAL FUNDS」で: 投信合計・既知資産に占める割合・毎月積立合計・戦術枠/FIRE Core比(許容内/やや大きめ/大きい/超過の帯)・評価額の鮮度(7日超でstale表示)。★入力方法: Watchlistで投信の口数を入れると日次NAV(投信ライブラリー)で自動追跡(リアルタイム不要)。口数が無い場合は現在評価額の手動入力でOK(入力日でstale判定)。口座区分(NISA/iDeCo/特定)・毎月積立額もここで入力。コスト未入力なら損益は表示しない(捏造なし)。★戦略連動: FIRE Core未入力ならFIRE整合は判定保留のまま/戦術枠が本丸に対して大きければ警告/個別株の利益はFIRE Coreへ移す候補として提示。★全データ(ファンド名・口数・評価額・積立額・口座区分)は端末内+暗号化バックアップのみ。証券会社ログイン・口座連携なし。将来見込みの精密計算なし。免許業の助言ではない。' },
