...
class RegistryAndCoverageTest(unittest.TestCase):
    def test_canonical_registry_is_sealed_and_originals_are_exact(self):
        self.assertEqual(
            jp_market_engine.CANONICAL_JP_MARKET_ENGINE_RFC_SHA256,
            "69a631ebc549b3bede6356cabf338e38d9418fc3683821198ef9a3c1eb440d51",
        )
        self.assertEqual(
            jp_market_engine.JP_MARKET_ENGINE_REGISTRY_SHA256,
            "f5771b1d1b4ca86f88aef02da91194078983f72b6ee4939acd38607494c6870b",
        )
        registry = jp_market_engine.sealed_proposition_registry()
        self.assertEqual(jp_market_engine.validate_proposition_registry(registry), (True, "valid"))
        ...
        self.assertEqual(jp_market_engine.validate_proposition_registry(registry),
                         (False, "registry_not_exact_sealed_value"))
        self.assertNotEqual(jp_market_engine.sealed_proposition_registry()["propositions"][0]["claim"],
                            "tampered")

    def test_coverage_artifact_locks_exact_credit_csv(self):
        artifact = jp_market_engine.coverage_artifact()
        ...
        self.assertEqual(
            artifact["artifactId"],
            "jp-market-engine-coverage-bd61cd1e6418f61ff3b67747402d50e0c48c667dc9b87f031272a47913d60f26",
        )
        committed = json.loads(
            (Path(__file__).parent / "artifacts" /
             "round2-jp-market-engine-registry-coverage-v1.json").read_text(encoding="utf-8"))
...
class PointInTimeAndPropositionTest(unittest.TestCase):
    def test_pit_excludes_future_missing_time_and_selects_revision(self):
        ...
        selected, proof = jp_market_engine.point_in_time_rows(rows, CUTOFF)
    ...
    def test_revision_without_own_known_at_is_never_admitted(self):
        ...
        selected, proof = jp_market_engine.point_in_time_rows([original, revision], CUTOFF)
    ...
    def test_future_observation_date_is_excluded_even_if_known_early(self):
        ...
        selected, proof = jp_market_engine.point_in_time_rows([row], CUTOFF)
        self.assertEqual(selected, [])
        self.assertEqual(proof["excludedFutureCount"], 1)
        self.assertFalse(proof["futureRowsAdmitted"])
    ...
    def test_d01_exact_boundary_and_candidates(self):
        ...
        result = jp_market_engine.evaluate_d01(rows, cutoff=CUTOFF)
        ...
        self.assertTrue(jp_market_engine.evaluate_d01(rows, cutoff=CUTOFF)["conditionMet"])

    def test_d02_never_infers_missing_1570_ratio(self):
        self.assertEqual(jp_market_engine.evaluate_d02([], cutoff=CUTOFF)["status"], "MISSING")
        one = jp_market_engine.evaluate_d02(
    ...
    def test_direct_relative_strength_precedes_proxy_candidate(self):
        ...
        chosen = jp_market_engine.evaluate_d03(
            cutoff=CUTOFF, direct_evidence=direct, proxy_evidence=proxy)
        self.assertEqual(chosen["sourceType"], "DIRECT_INDEX")
        proxy_only = jp_market_engine.evaluate_d03(cutoff=CUTOFF, proxy_evidence=proxy)
    ...
    def test_foreign_flow_is_publication_gated(self):
        ...
        self.assertEqual(jp_market_engine.evaluate_d05([row], cutoff=CUTOFF)["status"], "MISSING")
        row["availableFrom"] = "2026-08-02T00:00:00Z"
        self.assertEqual(jp_market_engine.evaluate_d05([row], cutoff=CUTOFF)["direction"], "INFLOW")
    ...
    def test_d06_preserves_unknown_original_parameter(self):
        ...
        result = jp_market_engine.evaluate_d06(rows, cutoff=CUTOFF)
        self.assertEqual(result["originalParameter"], "UNKNOWN")
        self.assertEqual(result["jpMarketEngineOriginalTransition"], None)
    ...
    def test_d07_does_not_synthesize_earnings_quality(self):
        ...
        result = jp_market_engine.evaluate_d07(
...
class StrictTechnicalAndTargetsTest(unittest.TestCase):
    def test_complete_ohlcv_never_fills_missing_components(self):
        source = bars(count=3)
        del source[1]["open"]
        normalized = jp_market_engine.normalize_complete_ohlcv(source, cutoff=CUTOFF)
    ...
    def test_reversal_state_supersedes_lagging_downside_and_may_jump(self):
        ...
        state = jp_market_engine.classify_reversal_state(
    ...
    def test_reversal_evidence_is_deterministic_and_strict(self):
        ...
        first = jp_market_engine.reversal_evidence(
            cutoff=CUTOFF, nikkei_rows=nikkei, vix_rows=vix)
        second = jp_market_engine.reversal_evidence(
            cutoff=CUTOFF, nikkei_rows=nikkei, vix_rows=vix)
        ...
        self.assertEqual(details["jpMarketEngineOriginalParameters"], "UNKNOWN")
        self.assertEqual(details["fakeCrossProbability"], None)
        proxy_only = jp_market_engine.reversal_evidence(
    ...
    def test_short_canonical_reversal_artifact_roundtrips_data_gated(self):
        short = jp_market_engine.build_reversal_engine(
            cutoff=CUTOFF, analysis_instrument="NIKKEI_225_INDEX",
        ...
        self.assertEqual(short["evidence"]["sarBullishFlip"]["status"],
                         "MISSING")
        self.assertEqual(jp_market_engine.validate_reversal_artifact(short), short)
    ...
    def test_target_zones_withhold_all_prevalidation_statistics(self):
        ...
        target = jp_market_engine.build_target_zones(
    ...
    def test_nikkei_valuation_targets_never_apply_to_1321(self):
        ...
        self.assertIn("nikkei_valuation_not_applicable_to_proxy_or_topix",
                      target["missing"])
        mislabeled = jp_market_engine.build_target_zones(
            cutoff=CUTOFF, analysis_instrument="NIKKEI_225_INDEX",
...
class IdentityStockLensAndProjectionTest(unittest.TestCase):
    def test_proxy_is_never_substituted_for_direct_index(self):
        ...
        model = jp_market_engine.build_direct_index_model(
            cutoff=CUTOFF, analysis_instrument="NIKKEI_225_INDEX",
        ...
        direct = jp_market_engine.build_direct_index_model(
            cutoff=CUTOFF, analysis_instrument="NIKKEI_225_INDEX",
    ...
    def test_provenance_rejects_unsupported_observed_and_preserves_inference(self):
        ...
        result = jp_market_engine.validate_evidence_provenance(
    ...
    def test_stock_lens_uses_required_hierarchy_and_no_inferred_driver(self):
        ...
        lens = jp_market_engine.build_stock_lens(
        ...
        self.assertEqual(lens["hierarchy"], [
            "JP_MARKET_ENGINE_JP_MARKET_STATE", "SECTOR_STYLE_STATE",
    ...
    def test_today_sda_projection_accepts_exact_artifacts_only(self):
        ...
        self.assertEqual(
            jp_market_engine.validate_reversal_artifact(reversal), reversal)
        ...
        hostile["evidenceArtifact"]["artifactId"] = \
            "jp-market-engine-reversal-evidence-" + jp_market_engine._sha256(evidence_body)
        ...
        hostile_body = {key: value for key, value in hostile.items()
                        if key != "artifactId"}
        hostile["artifactId"] = "jp-market-engine-reversal-" + jp_market_engine._sha256(hostile_body)
        with self.assertRaisesRegex(
                ValueError, "invalid_reversal_evidence_artifact"):
            jp_market_engine.validate_reversal_artifact(hostile)
        for axis in ("downsideAxis", "reversalAxis"):
            ...
            hostile_axis["artifactId"] = \
                "jp-market-engine-reversal-" + jp_market_engine._sha256(hostile_axis_body)
            with self.assertRaises(ValueError):
                jp_market_engine.validate_reversal_artifact(hostile_axis)
        ...
        hostile_proof["artifactId"] = \
            "jp-market-engine-reversal-" + jp_market_engine._sha256(hostile_body)
        with self.assertRaisesRegex(
                ValueError, "invalid_reversal_evidence_artifact"):
            jp_market_engine.validate_reversal_artifact(hostile_proof)
        ...
        pit_body = {
            ...
            "datasetHash": jp_market_engine._sha256([]),
        }
        pit = {**pit_body, "proofId": "jp-market-engine-pit-" + jp_market_engine._sha256(pit_body)}
        ohlcv_body = {
        ...
        zero_proof = {
            **ohlcv_body,
            "proofId": "jp-market-engine-ohlcv-" + jp_market_engine._sha256(ohlcv_body),
        }
        ...
        zero_coverage["evidence"] = zero_coverage[
            "evidenceArtifact"]["factors"]
        zero_coverage["reversalAxis"] = jp_market_engine.classify_reversal_state(
        ...
        zero_coverage["artifactId"] = \
            "jp-market-engine-reversal-" + jp_market_engine._sha256(zero_body)
        with self.assertRaisesRegex(
                ValueError, "invalid_reversal_evidence_artifact"):
            jp_market_engine.validate_reversal_artifact(zero_coverage)
        target = jp_market_engine.build_target_zones(
            cutoff=CUTOFF, analysis_instrument="NIKKEI_225_INDEX",
        ...
        rejected = jp_market_engine.project_today_sda_safe(cutoff=CUTOFF, reversal=tampered)
    ...
    def test_facade_is_json_serializable_and_has_no_action(self):
        result = jp_market_engine.evaluate_d01_d07(cutoff=CUTOFF)
