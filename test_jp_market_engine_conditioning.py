"""v13.5.14 — JP_MARKET_ENGINE-conditioned forecast engine (owner spec 2026-08-22).

The chart forecast is JP_MARKET_ENGINE's thinking routine: the analog search conditions on
...
def test_jp_market_engine_context_conditions_the_forecast():
    bars = _bars()
    plain = ti.calibrate_forecast(bars)
    conditioned = ti.calibrate_forecast(bars, jp_market_engine_context=_context(bars),
                                        market="JP")
    meta = conditioned["marketConditioning"]
    ...
    assert plain["marketConditioning"]["requested"] is False
...
def test_jp_close_cannot_see_same_evening_us_prints():
    ...
    jp = ti._jp_market_engine_daily_features(ti.normalize_bars(bars), context, "JP")
    us = ti._jp_market_engine_daily_features(ti.normalize_bars(bars), context, "US")
...
def test_absent_state_is_a_different_situation_not_zero():
    ...
    conditioned = ti.calibrate_forecast(bars, jp_market_engine_context=context, market="JP")
    meta = conditioned["marketConditioning"]
...
def test_analyze_pit_filters_jp_market_engine_context_and_stays_verified():
    bars = _bars(400)
    as_of = bars[-1]["date"] + "T23:59:59Z"
    clean = ti.analyze(bars, symbol="1321", market="JP",
                       jp_market_engine_context=_context(bars), as_of=as_of)
    ...
    poisoned = ti.analyze(bars, symbol="1321", market="JP",
                          jp_market_engine_context=poisoned_context, as_of=as_of)
    assert clean["pointInTime"]["verified"] is True
    assert set(clean["pointInTime"]["proofs"]) == {
        "bars", "shortSelling", "comparison"}
    assert clean["calibration"]["marketConditioning"]["requested"] is True
...
def test_conditioned_forecast_is_deterministic():
    ...
    left = ti.calibrate_forecast(bars, jp_market_engine_context=context, market="JP")
    right = ti.calibrate_forecast(bars, jp_market_engine_context=context, market="JP")
    assert left == right
...
def test_source_misconfiguration_is_reported_not_silent(monkeypatch):
    ...
    result = ti.calibrate_forecast(
        bars, jp_market_engine_context=context, market="JP")
    meta = result["marketConditioning"]
    assert meta["sourceIssues"] == ["vix_provider_key_missing"]
    clean = ti.calibrate_forecast(
        bars, jp_market_engine_context={"creditRows": [], "vixRows": [], "usRows": []},
        market="JP")
    assert clean["marketConditioning"]["sourceIssues"] == []
...
def test_input_freshness_tells_interval_from_failure_from_not_applicable():
    ...
    stale = ti.jp_market_engine_input_freshness(ctx, "2026-09-07", "JP")
    ...
    joined = ti.jp_market_engine_input_freshness(ctx, "2026-09-07", "JP")["credit"]
    ...
    pending = ti.jp_market_engine_input_freshness(ctx, "2026-09-07", "JP")["credit"]
    ...
    assert ti.jp_market_engine_input_freshness(ctx, "2026-09-07", "US")["credit"] == {"status": "not_applicable"}
    assert ti.jp_market_engine_input_freshness({"creditRows": []}, "2026-09-07", "JP")["credit"] == {"status": "no_rows"}
    ...
    report = ti.calibrate_forecast(bars, jp_market_engine_context=ctx, market="JP")
    assert report["marketConditioning"]["inputs"]["credit"]["status"] in ("not_yet_available", "joined", "stale_beyond_window")
    assert report["marketConditioning"]["inputs"]["vix"]["status"] in ("joined", "not_yet_available", "stale_beyond_window")
