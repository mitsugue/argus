...
def api_argus_dashboard_events():
    ...
    if items:
        ...
        try:
            bad = 0
            for it in items:
                t = it.get("eventTimeUtc")
                if not t or _parse_iso_epoch(str(t)) is None:
                    bad += 1
            _EVENTS_BUILD_LAST["dateIssues"] = bad
        except Exception:
            pass
...
_DOWNSIDE_THEMES = {
    ...
    # v12.0.8: 6965 浜松ホトニクス(光半導体/光電子デバイス)を追加 — AI半導体
    # バリューチェーン懸念での連動(オーナー実測)をテーマ連想として拾えるように。
    "ai_semis_cable": {"5801", "5803", "285A", "6920", "6857", "6965", "NVDA", "SMH", "AVGO", "TSM"},
    # 防衛/重工 — 地政学ニュースでの連動枠(v12.0.8)。
    "defense_heavy": {"7011", "7013"},
}
# v12.0.8 Part A: テーマ→連想語(OSINT帰属のsector_theme判定に使用。断定には使わない)
_THEME_WORDS_JA = {
    "ai_semis_cable": ["AI", "半導体", "AI半導体", "エヌビディア", "NVIDIA", "Samsung",
                        "サムスン", "Anthropic", "データセンター", "光半導体", "HBM",
                        "光電子", "フォトニクス", "AIチップ", "電線"],
