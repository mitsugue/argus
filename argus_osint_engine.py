...
_DIRECT_SUFFIX_JA = ("IR", "適時開示", "決算", "業績予想", "格付け", "受注", "提携",
                     "規制", "ニュース", "製品発表", "newsroom")
...
RESOLUTION_STATUSES = ("verified_integrated", "duplicate_existing", "stale_rejected",
                       ...
                       "hypothesis_not_source", "search_direction_only",
                       "inference_only", "still_unresolved_important")
RESOLUTION_JA = {
    ...
    "hypothesis_not_source": "未検証仮説(ソースではない)",
    "search_direction_only": "探索方向(クエリとして学習)",
    "inference_only": "推論のみ(注意書き必須)",
...
def resolve_gap(claim: Dict[str, Any], provider: str, argus_clusters: List[Dict[str, Any]],
    ...
    # ③b v12.1.4 Phase 1: 仮説/探索方向/推論はソース欠落と混同しない —
    # URL/日付分岐より先に判定(「〜の可能性」がmissing_urlや未回収重要に化けない)
    claim_type = classify_agent_claim(claim)
    if status is None and claim_type in _CLAIM_TYPE_TO_RESOLUTION:
        status, reason = _CLAIM_TYPE_TO_RESOLUTION[claim_type]
    # ④ URL/日付なし
...
def research_power_score(*, verified: List[Dict[str, Any]],
    ...
    # v12.1.4: 仮説規律/回収/新鮮度の成分入力(gap台帳から導出)
    hyp_handled = sum(1 for g in (gap_ledger or []) if g.get("resolutionStatus") in
                      ("hypothesis_not_source", "search_direction_only",
                       "inference_only"))
    ...
    components = {
        ...
        "hypothesisDisciplineScore": min(8, hyp_handled * 2),
        "blockedSourceRecoveryScore": min(6, int(recovery.get("attempted") or 0) * 2),
        "alternateSourceRecoveryScore": min(6, int(recovery.get("recovered") or 0) * 3),
        "freshNewsLatencyScore": min(6, fresh_verified * 3),
        ...
        "hallucinationResistanceScore": max(
    ...
    warnings_ja = []
    if hyp_handled >= 3:
        warnings_ja.append(f"未検証仮説{hyp_handled}件 — ソース欠落ではないが確度は控えめに")
    strengths = []
    if hyp_handled:
        strengths.append(f"仮説{hyp_handled}件を仮説として分離(未回収と混同しない)")
    if recovery.get("recovered"):
        strengths.append(f"参照不能ソース{recovery['recovered']}件を代替経路で回収")
    if official:
...
# ━━━ v12.1.4 Phase 1 — Agent Claim Taxonomy(仮説をソース欠落と混同しない) ━━━

AGENT_CLAIM_TYPES = ("concrete_source_claim", "official_disclosure_claim",
                     "market_commentary_claim", "hypothesis", "search_direction",
                     "causal_inference", "value_chain_inference",
                     "sector_theme_inference", "unsupported_narrative",
                     "price_only_narrative")

_HYPOTHESIS_MARKERS_JA = ("可能性", "連想", "かもしれない", "とみられる", "思われる",
                          "推測", "観測", "候補", "ようだ", "噂")
_HYPOTHESIS_MARKERS_EN = ("likely", "possible", "possibly", "may ", "might",
                          "could ", "seems", "sympathy", "market may be",
                          "related to", "perhaps")
_SEARCH_DIRECTION_MARKERS = ("探索", "調べる", "確認すべき", "search", "check ",
                             "look for", "からの連想", "による連想", "方向")
_CAUSAL_MARKERS = ("ため", "により", "要因", "反応した", "materialized", "because",
                   "driven by", "起因")
_PRICE_ONLY_MARKERS = ("株価が", "上昇した", "下落した", "急騰", "急落", "物色")


def classify_agent_claim(claim: Dict[str, Any]) -> str:
    """外部AI claimの型分類。URL+日付つきの具体主張だけがソース候補。
    「〜の可能性」「〜連想」等はソース欠落ではなく仮説/探索方向として扱う。"""
    title = str(claim.get("titleJa") or claim.get("title") or "")
    tl = title.lower()
    has_url = bool(claim.get("url"))
    has_date = claim.get("publishedAt") not in (None, "", "unknown")
    has_source = bool(claim.get("sourceName"))
    dirn = str(claim.get("directness") or "")

    hyp = any(m in title for m in _HYPOTHESIS_MARKERS_JA) or         any(m in tl for m in _HYPOTHESIS_MARKERS_EN)
    sdir = any(m in title for m in _SEARCH_DIRECTION_MARKERS) or         any(m in tl for m in _SEARCH_DIRECTION_MARKERS)

    # URL+日付が揃っていれば言い回しに関係なく具体ソース主張
    if has_url and has_date:
        if dirn == "direct_company" or claim.get("sourceType") in (
                "official_disclosure", "official_gov"):
            return "official_disclosure_claim"
        return "concrete_source_claim"
    if has_url or (has_source and has_date):
        # 一部の手掛かりあり — 仮説語がなければ具体主張として追跡
        if not hyp and not sdir:
            return "concrete_source_claim"
    if sdir and not has_url:
        return "search_direction"
    if hyp and not has_url:
        if dirn == "value_chain":
            return "value_chain_inference"
        if dirn == "sector_theme":
            return "sector_theme_inference"
        return "hypothesis"
    if any(m in title for m in _PRICE_ONLY_MARKERS) and not has_url and not has_source:
        return "price_only_narrative"
    if any(m in title for m in _CAUSAL_MARKERS) and not has_url and not has_source:
        return "causal_inference"
    # ヘッジ語も値動き語もない主張は具体ソース主張として追跡する
    # (「SEAJが予測を上方修正」のような事実主張を物語扱いして却下しない —
    #  URL/日付の欠落はresolve_gapのmissing_url/missing_date側が正確に扱う)
    return "concrete_source_claim"


_CLAIM_TYPE_TO_RESOLUTION = {
    "hypothesis": ("hypothesis_not_source",
                   "未検証仮説 — ソース欠落ではない(確度は下げる・優位性はブロックしない)"),
    "sector_theme_inference": ("hypothesis_not_source",
                               "テーマ連想の仮説 — 事実として扱わない(優位性はブロックしない)"),
    "value_chain_inference": ("inference_only",
                              "バリューチェーン推論 — 注意書き必須(優位性はブロックしない)"),
    "causal_inference": ("inference_only",
                         "ソースなしの因果推論 — 単独ではGemini超過をブロックしない"),
    "search_direction": ("search_direction_only",
                         "探索方向 — 追撃クエリとして学習(ソース欠落ではない)"),
    "unsupported_narrative": ("unsupported_rejected",
                              "裏付けのない物語 — 証拠にしない"),
    "price_only_narrative": ("unsupported_rejected",
                             "値動きだけの物語 — 原因をでっち上げない"),
}


# ━━━ v12.1.4 Phase 3 — Blocked Source Recovery(参照不能ソースの代替回収) ━━━

PRESS_SOURCE_CATEGORIES = ("press_release", "press_release_aggregator",
                           "company_newsroom", "product_release", "product_page",
                           "metadata_only")

_PR_AGGREGATOR_DOMAINS = ("prtimes.jp", "dreamnews.jp", "atpress.ne.jp",
                          "kyodonewsprwire.jp", "value-press.com")


def press_source_category(url: str, source_name: str = "") -> Optional[str]:
    u = str(url or "").lower()
    sn = str(source_name or "").lower()
    if any(d in u or d in sn for d in _PR_AGGREGATOR_DOMAINS):
        return "press_release_aggregator"
    if "ir" in u.split("/")[-1:] or "/news" in u or "newsroom" in u:
        return "company_newsroom"
    if "プレスリリース" in str(source_name or "") or "press release" in sn:
        return "press_release"
    return None


def blocked_source_recovery_queries(gap: Dict[str, Any],
                                    company_ja: str = "",
                                    company_en: str = "") -> List[str]:
    """参照不能(bot遮断/ペイウォール)ソースの代替探索クエリ。
    合法的な公開経路のみ: 同一ニュースの企業ニュースルーム/別アグリゲータ/
    タイトル再検索。ペイウォール迂回や強引なスクレイピングはしない。"""
    title = str(gap.get("sourceTitle") or "")[:80]
    out = []
    if title:
        out.append(title)                            # ドメイン抜きタイトル再探索
    if company_ja:
        out.append(f"{company_ja} ニュースルーム")
        out.append(f"{company_ja} プレスリリース")
        if title:
            ents = sorted(_entities(title))[:2]
            for e in ents:
                out.append(f"{company_ja} {e}")
    if company_en and title:
        out.append(f"{company_en} news")
    seen, dedup = set(), []
    for q in out:
...
# ━━━ v12.1.4 Phase 4 — Fresh News Acceleration(予備状態の明示) ━━━━━━━━━━━

PRELIMINARY_STATUSES = ("fresh_candidate", "metadata_only",
                        "awaiting_verification", "verified", "rejected")


def preliminary_status(vs: Dict[str, Any]) -> str:
    """検証前の新鮮候補を明示。予備候補は決定的な原因になれない
    (primaryEligibleは検証済みのみ — ここでは表示状態だけを付ける)。"""
    st = vs.get("verificationStatus")
    if st == "verified":
        return "verified"
    if st in ("stale", "contradicted"):
        return "rejected"
    if st == "metadata_only":
        return ("fresh_candidate"
                if vs.get("freshness") in ("today", "within_3_trading_days")
                else "metadata_only")
