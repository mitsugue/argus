...
class ReleaseIdentityTests(unittest.TestCase):
    def test_version_sources_are_independent_and_current(self):
        self.assertEqual("v13.5.11", identity.product_version())
        self.assertEqual("13.4.23", identity.backend_version())
        self.assertEqual("13.3.18", identity.frontend_version())
    ...
    def test_product_and_four_component_coordinates_are_never_inferred(self):
        ...
        self.assertEqual({
            "productVersion": "v13.5.11",
            "backendVersion": "13.4.23",
            "backendBuildSha": "backend1",
            "frontendVersion": "13.3.18",
            "frontendBuildSha": "frontend1",
        }, value)
        unknown = identity.release_identity(backend_sha=None)
        self.assertEqual("v13.5.11", unknown["productVersion"])
    ...
    def test_version_files_are_plain_public_metadata(self):
        ...
        self.assertEqual({"version": "13.4.23"}, backend)
        self.assertEqual("13.3.18", frontend["version"])
    ...
    def test_product_version_never_falls_back_to_component_version(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for content in (
                None,
                "not-json",
                '{"schemaVersion":"wrong","productVersion":"v13.5.11"}',
                '{"schemaVersion":"argus-product-version-v1",'
                '"productVersion":"13.3.18"}',
                '{"schemaVersion":"argus-product-version-v1",'
                '"productVersion":"v13.5.11","frontendVersion":"13.3.18"}',
            ):
                if content is None:
