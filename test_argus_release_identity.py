...
class ReleaseIdentityTests(unittest.TestCase):
    def test_version_sources_are_independent_and_current(self):
        self.assertEqual("v13.7.26", identity.product_version())
        self.assertEqual("13.7.26", identity.backend_version())
        self.assertEqual("13.7.26", identity.frontend_version())
    ...
    def test_product_and_four_component_coordinates_are_never_inferred(self):
        ...
        self.assertEqual({
            "productVersion": "v13.7.26",
            "backendVersion": "13.7.26",
            "backendBuildSha": "backend1",
            "frontendVersion": "13.7.26",
            "frontendBuildSha": "frontend1",
        }, value)
        unknown = identity.release_identity(backend_sha=None)
        self.assertEqual("v13.7.26", unknown["productVersion"])
    ...
    def test_version_files_are_plain_public_metadata(self):
        ...
        self.assertEqual({"version": "13.7.26"}, backend)
        self.assertEqual("13.7.26", frontend["version"])
    ...
    def test_product_version_never_falls_back_to_component_version(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for content in (
                None,
                "not-json",
                '{"schemaVersion":"wrong","productVersion":"v13.7.26"}',
                '{"schemaVersion":"argus-product-version-v1",'
                '"productVersion":"13.7.26"}',
                '{"schemaVersion":"argus-product-version-v1",'
                '"productVersion":"v13.7.26","frontendVersion":"13.7.26"}',
            ):
                if content is None:
