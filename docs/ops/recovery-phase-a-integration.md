...
## 同一入力の市場解説再利用（2026-09-15）

市場解説の10分ごとの確認で、生成時刻から派生するチャートIDを作る前に、既存の入力を照合する。ニュース等の合成根拠、日経平均の原系列、全市場条件履歴、市場内部の計算結果、評価尺度と取得状態、実モデル・実行コード・設定を含める。取得日時・原応答ハッシュ・訂正・欠測・未知の追加項目は残す。除くのはローカルの文章生成時刻・内部比較確認時刻とキャッシュ期限だけ。

同一のUTC時刻帯内に限り、完全に同じ入力なら元の説明・チャート・生成時刻・保存履歴を返す。生成待ちの間に入力が変わった場合は再利用を承認しない。キャッシュ中の未来日時に到達した場合も再照合する。再利用結果の`generationReuse.inputsCheckedAt`を新しい確認時刻とし、元の生成日時を書き換えない。再起動後はこのメモリ内比較を引き継がず再評価する。既存の永続履歴は保持する。

これは原典の再取得自体を同一視する最適化ではない。同じ数値でも取得日時・応答ハッシュが変われば再評価する保守的な実装である。新しい原典、イベント・保有側の処理や明示的な質問を止めない。既存の指標計算・閾値・売買権限は変更しない。実際の再利用回数、長期の費用削減、原典再取得時のさらなる重複削減は未確認として分ける。

## GPT送信時の参照ID短縮（2026-09-15）

市場解説の根拠・前回根拠・表示候補で繰り返す長い根拠IDを、1回の要求だけに閉じた短い参照へ置き換える。事実の本文、数値、単位、時点、出典、検証区分、候補件数は削らない。対応表はサーバー側の要求内に保持し、回答の参照を元へ戻してから従来の根拠・数値・表示検査を実行する。保存する文脈・説明・チャート・履歴のIDは変更しない。未知の参照は検査で拒否し、本文中の文字は参照として置き換えない。修正要求でも同じ対応を使う。

本番から取得した保存済み文脈1件では、根拠と表示候補JSONは52,106文字から28,754文字となり、元の文脈・表示候補への完全復元を確認した。新規生成・本番費用削減の実証ではなく、文字数の比較である。直近には市場解説で60秒のAPIタイムアウトが複数あり、成功応答の入力は約2.4万〜2.9万トークンだった。短縮後の応答時間・実トークン・採用結果・費用は配信後に観測する。再試行の無制限化や必要な根拠の削除では解決しない。
