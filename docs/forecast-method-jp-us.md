...
> （`argus_today_intelligence.JP_MARKET_ENGINE_FEATURE_SCALES`）にそのような特徴は存在しない。米国の条件付けは
...
|---|---|---|---|---|
| 7 ファミリー D01〜D07（需給・トレンド） | D01 二市場信用残（JPX CSV + 台帳）、D02 1570 信用倍率（J-Quants 週次）、D03 日米相対力（1321 vs SPY 20 営業日）、D04 EPS/PER（ライセンス未取得＝欠測）、D05 海外投資家フロー（台帳）、D06 VIX 水準/10 日変化（Yahoo ^VIX）、D07 決算反応（J-Quants fins/summary + 日足） | `jp_market_engine.evaluate_d01_d07`：各ファミリーの条件成立/不成立/判定不能を命題台帳（sealed registry）どおりに決定論で評価 | `families[D0x].conditionMet`、`marketSignals SIG-01..07 activeCount/7` | 命題台帳の SHA 固定、`test_jp_market_engine*`、本番の sourceStatus で各入力の供給元を明示 |
| 反転/下方（市場観） | ^N225 日足、^VIX 日足 | `jp_market_engine.build_reversal_engine`（反転: RECOVERY_TEST 等、下方: MIXED 等） | `reversal.reversalState / downsideState` | 行動権限なし（`actionAuthority=false`）。SDA 入力にしない |
| チャート予測（上限/下限/本線/無効・UP/RANGE/DOWN） | 判断主体の日足（J-Quants 10 年）、JP_MARKET_ENGINE 状態の日次系列（creditRatio, creditShortTn, vixLevel, vixChange10, rs20） | `argus_today_intelligence`（today-replay-calibration-v3-market-conditioned）：JP_MARKET_ENGINE 状態でスケールした特徴空間で類似局面 kNN → 1/5/20 営業日先の終値方向の出現頻度・ATR14 の帯・支持抵抗 | `directionProbabilities`（頻度）、`upside/downside/invalidation`、`marketConditioning{currentFeatureKeys, coverageDays, sourceIssues}` | walk-forward Brier / BSS、独立 holdout。現在 BSS ≈ −0.003〜−0.007 ＝ **予測力未証明** → 画面は「検証済み確率ではない」と表示、BUY は構造的に無効 |
| 切替の明示 | JP_MARKET_ENGINE 状態の全特徴が取れない日 | 無条件の類似局面へ切替 | `marketConditioningJa` = 「JP_MARKET_ENGINE条件なし（無条件の類似局面）」または「JP_MARKET_ENGINE条件を取得できず無条件の類似局面へ切替中」 | 供給設定障害（例: FRED 鍵未設定）は `sourceIssues` で名指し |
...
| 反転/下方 | 適用外 | — | 米国では表示しない | — |
| チャート予測 | SPY/QQQ 日足（Twelve Data）。条件付け特徴は **VIX 水準（`vixLevel`）・VIX 10 日変化（`vixChange10`）・対 SPY 20 営業日相対力（`rs20`、SPY 自身は無し）**。信用残（`creditRatio`/`creditShortTn`）は米国では結合しない（`creditRows=[]`） | 同じ replay エンジン（today-replay-calibration-v3-market-conditioned）。米国の足は同日の VIX を参照できる（知識ラグなし） | `marketConditioning.currentFeatureKeys` ⊆ {vixLevel, vixChange10, rs20}、画面は「米国の市場状態で条件付け: VIX水準・VIX10日変化・対SPY相対力」 | 同上（BSS 未証明） |
