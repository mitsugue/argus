...
deployment alone would not activate the new backend evidence fields.

The owner explicitly approved interrupting the already-invalidated v13.3.2
observation on 2026-07-29 after production evidence showed a Render OOM restart,
an oversized watchtower ledger blob, and repeated scheduled-workflow failures.
The rollout therefore starts a new v13.3.3 72-hour soak; elapsed time from the
v13.3.2 run is not carried forward.
