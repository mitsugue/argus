...
この段階は既存Recovery/Product両証明でサーバー接続を配信する候補。画面の質問欄の再接続と生成中/失敗の区別は次の番号更新を含むProduct配信へ引き継ぐ。関連API・暗号化復元・隔離ブラウザは確認済み。本番の新経路・私的履歴とiPhoneの受入は未完了。13.7には進まない。
