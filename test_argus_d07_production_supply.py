"""v13.5.36 — JP_MARKET_ENGINE D07 production supply (earnings response evidence).
...
def _seed(rows, monkeypatch):
    monkeypatch.setitem(scanner._JP_MARKET_ENGINE_STATEMENTS_CACHE, "rows", rows)
    monkeypatch.setitem(scanner._JP_MARKET_ENGINE_STATEMENTS_CACHE, "source",
...
def test_before_open_disclosure_same_day_without_flag(monkeypatch):
    ...
    event, _ = scanner._jp_market_engine_earnings_event()
    assert event["date"] == "2026-08-25"
...
def test_unrecognized_schema_stays_missing(monkeypatch):
    ...
    verdict = jp_market_engine.evaluate_d07(
...
def test_no_lookahead_event_invisible_before_disclosure_instant():
    ...
    before = jp_market_engine.evaluate_d07(
    ...
    after = jp_market_engine.evaluate_d07(
    ...
    early = jp_market_engine.evaluate_d07(
...
def test_d07_missing_is_unknown_and_never_a_final_action():
    verdict = jp_market_engine.evaluate_d07(cutoff="2026-08-26T12:00:00Z")
    assert verdict["status"] == "MISSING"
    ...
    assert verdict["lineage"] == "JP_MARKET_ENGINE_ORIGINAL"
