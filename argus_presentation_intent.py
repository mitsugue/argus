...
def generation_instruction(catalog: Mapping[str, Any]) -> str:
    return ('ARGUSとしてアプリ全体を一つの意図で編集してください。読み手へ何を伝えるかを決め、'
        ...
        'この4キーが各要素の全項目で、evidence-で始まるidに限りcaptionを追加します。'
        'view/reasons/changes/impact/next/invalidation/nikkei-comparisonにはcaptionを付けません。nullも不可。'
        '候補のkind/payloadId/evidenceIds/mandatory/urgentは入力専用です。elementsの直下の要素へコピーしません。'
        '6項目の本文はトップレベルの同名オブジェクトへ置き、presentationの要素へtextJaや本文を追加しません。'
