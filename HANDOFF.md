...
- v10.35.0 運用正直化(GPT Proレビューの有効分のみ反映) — (#10)自己採点表示にsample-size注記
  ...
  closepin/market-alerts通知を正しく記載)。(#5)/api/argus/calibrationを_ledger_summary(本物)に接続
  (旧=Render揮発ファイルで常時ゼロのfootgun除去・deprecated明記)。GPTの他項目は既に対応済み
  (#1 versionはv10.33で根本修正・#2 closepin EC2発火確認済Mon初記録待ち・#11 secret監査=コミット履歴に
  ハードコード無し)か大型で後続(#3 quote entitlement・#4 AI fresh/stale UI・#5 Ledger Health画面・
  #9 scanner分割)。
...
- v10.33.0 バージョン表記スタック修正 — web/package.jsonがv10.30.0で止まっていた(v10.31以降の
  ...
  10.30固定だった。機能(v10.31〜10.32.1)は全てデプロイ済み・表記のみ誤り。今後bumpはEditツールで直接
  +dist/index.htmlの__ARGUS_VERSION__を必ず検証(教訓)。
...
- v10.28.0 スクロール操作改善+Catalysts整理 — AppShellオーバースクロールを双方向化(上=前ページ/下=次ページ)、
  ...
  _td_price_history失敗キャッシュ600→150s。米国scoutは本番でAAPL/MSFT/TSLA live確認(NVDA初回はTDレート制限の一時失敗)
- v10.27.0 米国株エントリー診断(us-scout-v1) — `_td_price_history`(Twelve Data 130日OHLCV・6h・1credit)。
  get_entry_scout(sym, market)でルーティング: US→_td履歴/USフロー/SPY相対力、JP専用信号(日証金/JPX空売り/margin)は
  Noneで正直表示。エンドポイントは?market=USで米国受付(_US_SYM_RE)。フロントは米国株にも⚡ボタン。
  _entry_metrics/flow_inference/catalystは市場非依存で再利用。87テスト。次候補: 米国短期金利/short-interest、第2層板/VWAP
